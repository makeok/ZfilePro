<template>
	...
	<div class="zfile-gallery-body">
		<div class="zfile-img-body" v-if="transferResult.length > 0">
			<div class="zfile-img-row" v-for="(rowItem, index) in transferResult" :key="index+'zfile-img-row'">
				<div class="zfile-img-col"
					...
					<template v-if="colItem?.url">
						<img class="zfile-gallery-img lazyload"
							...
							:data-src="colItem.thumbnail"
							:src="colItem.thumbnail"
							:key="md5(colItem.url)"
							loading="lazy"
							:alt="colItem.name"/>
...
const transferResult = computed(() => {
	...
	// 图片行转列完成后的数组
	let outImg =  transfer(imgArray.value);
	return outImg;
})
