		...
		<div class="h-full lg:h-auto max-w-screen-lg bg-white shadow-xl sm:rounded-xl flex flex-1">
            ...
            <div class="my-12 relative">
              ...
              <div class="relative flex justify-center text-sm">
                <span class="px-2 bg-white text-sm leading-none text-gray-600 tracking-wide font-medium">账号登录</span>
              </div>
            </div>
            ...
            <el-form @submit.native.prevent ref="loginFormRef" :rules="formRules" :model="formData"
							...
							<el-form-item class="box animate__animated animate__fadeInUp" prop="username">
								<el-input placeholder="账号" :prefix-icon="User" v-model.trim="formData.username"/>
							</el-form-item>

							<el-form-item class="box animate__animated animate__fadeInUp" prop="password">
								<el-input placeholder="密码" type="password" show-password :prefix-icon="Key" v-model.trim="formData.password"/>
							</el-form-item>
	...
	loginFormRef.value.validate((checked) => {
		if (checked) {
			...
			loginReq(formData.value).then((response) => {
				window.localStorage.setItem("zfile-token", response.data.token);
				window.localStorage.setItem("zfile-user", JSON.stringify(response.data.user));
				ElMessage({
					message: "登录成功",
					type: 'success',
					duration: 1000,
